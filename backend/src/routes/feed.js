...
const memories = [
  new Memory('An unforgetale day', "This day was awesome, I'll never forget it", new Date(1995, 11, 17), places[0]),
  new Memory(
...
router.get('/', (req, res) => {
  const result = memories.map(({ title, text, date, place: { name } }) => ({
    title,
    text,
